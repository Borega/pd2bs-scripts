...
var Town = {
	...
	tasks: [
		{Heal: NPC.Akara, Shop: NPC.Akara, Gamble: NPC.Gheed, Repair: NPC.Charsi, Merc: NPC.Kashya, Key: NPC.Gheed, CainID: NPC.Cain},
...
CursorLoop:
	...
	checkKeys: function () {
		...
		else if (!Config.OpenChests || me.classid === 6 || me.gold < 540 || (!me.getItem("key") && !Storage.Inventory.CanFit({sizex: 1, sizey: 1}))) {
			return 12;
		}
...
MainLoop:
		...
		if (items) {
			for (i = 0; i < items.length; i += 1) {
				if (this.canStash(items[i])) {
					...
					if (result) {
						if (Config.ItemInfoQuality.indexOf(items[i].quality) !== -1) {
	...
	initialize: function () {
		...
		case 2:
			...
			this.act[1].spot[NPC.Drognan] = [5090, 5035];
