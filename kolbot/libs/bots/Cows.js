...
function Cows() {
	this.buildCowRooms = function () {
		...
		kingPreset = getPresetUnit(me.area, 1, 893);
	...
	this.getLeg = function () {
		...
		if (me.getItem(NTIPAliasClassID["leg"])) {
			return me.getItem(NTIPAliasClassID["leg"]);
		}

		Pather.useWaypoint(4);
		Precast.doPrecast(true);
		Pather.moveToPreset(me.area, 1, 888, 10, 10);
		...
		for (i = 0; i < 8; i += 1) {
			...
			leg = getUnit(4, NTIPAliasClassID["leg"]);
